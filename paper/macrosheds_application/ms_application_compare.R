...
p_box <- n_frame %>%
    ...
    theme(text = element_text(size = 20),
          legend.position = 0)
#ggsave(file = here('paper', 'macrosheds_application', 'method_comp_fig.png'))
...
p_den <- out_tbl  %>%
    ...
    theme_classic()+
    theme(text = element_text(size = 20))
ggsave(file = here('paper', 'macrosheds_application', 'method_comp_den.png'))

# combine figures and save out
p_den|p_box
ggsave(file = here('paper', 'macrosheds_application', 'method_comp_combined.png'), width = 8, height = 6)
